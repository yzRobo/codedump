...
def should_skip(path):
    ...
    allowed_extensions = {
        # ... (no changes here, keeping it for brevity) ...
        # General
    ...
    allowed_filenames = {
        ...
        'tslint.json', 'webpack.config.js', 'yarn.lock', # MODIFIED: Removed 'package-lock.json'
        # Ruby
        'gemfile', 'rakefile',
        # ... (rest of the set is unchanged) ...
        # PHP
    ...
    # ADDED: A new set for specific filenames to skip
    skip_filenames = {
        'package-lock.json',
    }
    ...
    # ADDED: Check if the filename is in the explicit skip list
...
def concatenate_files(directory='.', list_only=False):
    ...
    for root, dirs, files in os.walk(directory):
        # This line correctly prunes the directories to search
        dirs[:] = [d for d in dirs if not should_skip(os.path.join(root, d))]

        for file in files:
            file_path = os.path.join(root, file)
            # IMPROVED: Pass the full file_path to should_skip for consistency and correctness
